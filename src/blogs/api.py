...
class BlogsAPI(GenericAPIView):
    sort_fields = ['name', 'description', "owner", "category"]

    def get(self, request):

        user_id = self.__filter_username()
        if user_id == USERNAME.NOT_INFORMED:
            self.queryset = Blog.objects.select_related("owner", "category").all()
        else:
            self.queryset = Blog.objects.select_related("owner", "category").filter(owner=user_id)
